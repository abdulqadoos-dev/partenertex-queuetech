            ...
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        <p class="mb-4">View all Inventories</p>
                    </div>
                    <div>
                        <a href="{{route('inventories.form')}}" class="btn btn-primary">Add New</a>
                    </div>
                </div>
                ...
                <table class="table table-borderless" id="data-table">
                    ...
                    @foreach($inventories as $item)
                        <tr>
                            ...
                            <th scope="row">
                                <a href="{{route('inventories.form',['id' => $item->id])}}">
                                    <i class="fa fa-pen-alt text-warning"></i>
                                </a>
                                <a href="javascript:void(0)" onclick="document.getElementById('form-{{$item->id}}').submit();">
                                    <i class="fa fa-trash-alt text-danger"></i>
                                    <form method="post" id="form-{{$item->id}}" action="{{route('inventories.delete', ['id' => $item->id])}}" onsubmit="return confirm('Are you sure you want to delete this')">
                                        @csrf
                                        @method('Delete')
                                    </form>
                                </a>
